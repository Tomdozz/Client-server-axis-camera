...
{  
    syslog(LOG_ERR,"installing server version 3");      
    int maxClients = 30;
    ...
    {   
	syslog(LOG_ERR, "Creation of master socket failed");
...
//  {   
//     syslog(LOG_ERR, "Failed to set master socket to reduce wait time.");
...
//  }   
    ...
    syslog(LOG_ERR, "waiting for connections...");
    ...
    {   
        ...
        {   
            ...
            syslog(LOG_ERR, "Error selecting activity");
        }
        ...
        {     
            ...
            {   
                ...
                syslog(LOG_ERR, "Error accepting new connection");
            }   
            printf("New connection, socket descriptor is %d, ip is: %s, port: %d\n", newSocket, inet_ntoa(address.sin_addr), ntohs(address.sin_port));
            syslog(LOG_ERR, "New connection, socket descriptor is %d, ip is: %s, port: %d\n", newSocket, inet_ntoa(address.sin_addr), ntohs(address.sin_port));
            ...
            if(send(newSocket, &message, sizeof(message), 0) != sizeof(message))
            {
                ...
                syslog(LOG_ERR, "Error in sending whole message");
            }
            ...
            for (i = 0; i < maxClients; i++)
            {
                if(clients[i].descriptor == -1)
                {
                    ...
                    syslog(LOG_ERR, "Adding to list of sockets as %d", i);
                    foundSpace = true;
                    break;
                }
            ...
            if (foundSpace == false)
            {
                ...
                syslog(LOG_ERR, "There's no space on the server for you!");
        ...
        {   
            ...
            {       
		        ...
		        syslog(LOG_ERR, "Characters read: %d", charactersRead);
                if (charactersRead <= 0)
                {   
                    ...
                    syslog(LOG_ERR, "Disconnected, ip: %s, port: %d \n", inet_ntoa(address.sin_addr), ntohs(address.sin_port));
                ...
                else
                {
                    ...
                    syslog(LOG_ERR, "Width: %d, Height: %d, Freq: %d ", clients[i].width, clients[i].height, clients[i].frequency);
                 
                    char imgDescription[100];
                    snprintf(imgDescription, sizeof(imgDescription), "resolution=%dx%d&fps=%d", clients[i].width, clients[i].height, clients[i].frequency);
                    syslog(LOG_ERR, imgDescription);
                    clients[i].stream = capture_open_stream(IMAGE_JPEG, imgDescription);
                    if (clients[i].stream == NULL) 
                    {
                        syslog(LOG_ERR, "Failed to open stream\n");
                    }
                }
            }
